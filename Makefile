...
integration-test: manifests generate fmt vet envtest ## Run integration tests.
	$(shell cat .env.integration 2>/dev/null | xargs) KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test ./... -covermode=atomic -coverpkg=all --tags=integration -coverprofile cover.out -args -skipUnconfiguredOneApp
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name bojanzelic-cloudflare-zero-trust-operator-migration-builder
	$(CONTAINER_TOOL) buildx use bojanzelic-cloudflare-zero-trust-operator-migration-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm bojanzelic-cloudflare-zero-trust-operator-migration-builder
	rm Dockerfile.cross
