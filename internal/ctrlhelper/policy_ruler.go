...
func (helper *ControllerHelper) PopulateWithCloudflareUUIDs(
	...
	for _, ruleSet := range ruleSets { //nolint:varnamelen
		...
		if err != nil {
			...
			return nil, fault.Wrap(err, fmsg.With("issue while extracting group refs"))
		}
		for _, accessGroupRef := range accessGroupRefs {
			...
			if accessGroup.Status.AccessGroupID == "" {
				...
				return &ctrl.Result{RequeueAfter: helper.NormalRequeueDelay}, nil
			}
		...
		if err != nil {
			// will retry immediately
			return nil, fault.Wrap(err, fmsg.With("issue while extracting service token refs"))
		}
