...
//
// @dev Might ask for a requeue, but only on error.
//
//nolint:gocognit
	...
	for i, rulesType := range managedCFRules { //nolint:varnamelen
		...
		for _, tokenRef := range tokenRefs {
			...
			if err := k8sCli.Get(ctx, tokenRef, serviceToken); err != nil {
				return ctrl.Result{}, errors.Wrapf(err, "unable to reference CloudflareServiceToken %s - %s", tokenRef.Name, tokenRef.Namespace)
			}

			//
			if serviceToken.Status.ServiceTokenID == "" {
				return ctrl.Result{RequeueAfter: 10 * time.Second},
					errors.Errorf("CloudflareServiceToken %s - %s not ready yet", tokenRef.Name, tokenRef.Namespace)
			}
